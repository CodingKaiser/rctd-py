...
# ---------------------------------------------------------------------------
# Triton kernel for fused spline interpolation (Phase 1 RAPIDS-inspired opt)
# ---------------------------------------------------------------------------
...
if _HAS_TRITON:
    ...
    def _calc_q_all_kernel(
        ...
        """Fused cubic spline interpolation kernel.

        Each thread processes one element: (Y[i], lam[i]) → (d0, d1, d2).
        """
        pid = tl.program_id(0)
        ...
        # Load inputs
        Y_raw = tl.load(Y_ptr + offsets, mask=mask, other=0.0)
        lam_raw = tl.load(lam_ptr + offsets, mask=mask, other=1e-4)

        # Clamp Y to [0, K_val]
        K_val_f = tl.full([1], K_val, dtype=Y_raw.dtype)
        Y_i = tl.minimum(Y_raw, K_val_f)
        Y_idx = Y_i.to(tl.int32)

        # Clamp lambda
        epsilon = 1e-4
        # X_max = x_vals[N_X - 1]
        X_max = tl.load(X_ptr + (N_X - 1))
        lam_i = tl.minimum(tl.maximum(lam_raw, epsilon), X_max - epsilon)

        # Grid index: l = floor(sqrt(lam / 1e-6))
        delta = 1e-6
        l_f = tl.math.sqrt(lam_i / delta)
        l = l_f.to(tl.int32)

        # m = min(l - 9, 40) + max(ceil(sqrt(max(l - 48.7499, 0) * 4)) - 2, 0)
        part1 = tl.minimum(l - 9, 40)
        l_float = l.to(tl.float32)
        inner = tl.maximum(l_float - 48.7499, 0.0) * 4.0
        part2 = tl.maximum(tl.math.ceil(tl.math.sqrt(inner)).to(tl.int32) - 2, 0)
        m = part1 + part2

        # Load x_vals[m-1] and x_vals[m]
        ti1 = tl.load(X_ptr + (m - 1), mask=mask, other=0.0)
        ti = tl.load(X_ptr + m, mask=mask, other=1.0)
        hi = ti - ti1

        # Gather from Q_mat and SQ_mat: row = Y_idx, col = m-1 or m
        # Q_mat layout: (K_val+3, N_X), stride0 = N_X
        q_offset_m1 = Y_idx * Q_stride0 + (m - 1)
        q_offset_m = Y_idx * Q_stride0 + m

        fti1 = tl.load(Q_ptr + q_offset_m1, mask=mask, other=0.0)
        fti = tl.load(Q_ptr + q_offset_m, mask=mask, other=0.0)
        zi1 = tl.load(SQ_ptr + q_offset_m1, mask=mask, other=0.0)
        zi = tl.load(SQ_ptr + q_offset_m, mask=mask, other=0.0)

        # Spline intermediates
        diff1 = lam_i - ti1
        diff2 = ti - lam_i
        inv_hi = 1.0 / hi
        diff3 = fti * inv_hi - zi * hi / 6.0
        diff4 = fti1 * inv_hi - zi1 * hi / 6.0
        zdi = zi * inv_hi
        zdi1 = zi1 * inv_hi
        ...
        d0 = (
            zdi * diff1_sq * diff1 / 6.0
            + zdi1 * diff2_sq * diff2 / 6.0
            + diff3 * diff1
            + diff4 * diff2
        )
        d1 = zdi * diff1_sq / 2.0 - zdi1 * diff2_sq / 2.0 + diff3 - diff4
        d2 = zdi * diff1 + zdi1 * diff2

        # Store outputs
        tl.store(d0_ptr + offsets, d0, mask=mask)
        tl.store(d1_ptr + offsets, d1, mask=mask)
        tl.store(d2_ptr + offsets, d2, mask=mask)

    def _calc_q_all_triton(
        Y: torch.Tensor,
        lam: torch.Tensor,
        Q_mat: torch.Tensor,
        SQ_mat: torch.Tensor,
        ...
        """Triton-accelerated cubic spline interpolation.

        Fuses the entire spline evaluation into a single kernel launch,
        ...
        d0 = torch.empty_like(lam)
        d1 = torch.empty_like(lam)
        d2 = torch.empty_like(lam)
